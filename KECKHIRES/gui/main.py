...
class UiBfosc(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.trace_handle = []
        self.strimy = [4, 4000] #strim pixal on y axis (ystart, yend) 
        self.strimx = [0, 713] #strim pixal on x axis (xstart, xend)
        self.strimdy = self.strimy[1] - self.strimy[0]
        self.strimdx = self.strimx[1] - self.strimx[0]
        self.ap_trace = np.zeros((0, self.strimdy), dtype=int)
        self._lamp = None
        self.aperture_image = None
        self.wavecalibrate = True
        # UI
        self.setupUi(self)
        self.add_canvas()
        self.initUi()
    ...
    def assumption(self):
        ...
        #self._lamp = joblib.load("/home/lcq/Documents/pyrafspec/data/template/template_20211023_order9.dump")
        #self.ap = joblib.load("../aperture_template/ap.dump")
        apfname = f'{self._wd}/ap-ihdu-{self.ihdu}.dump'
    ...
    def _make_datatable(self):
        ...
        for i in range(self.nfp):
            if "bias" in imgtype[i].lower() or "bias" in fps_full[i].lower():
                ...
                types[i] = "flat"
            elif "arcs" in imgtype[i].lower(): #and (exptime[i] == 300 or "arcs" in fps_full[i].lower()):
                types[i] = "lamp"
    ...
    def _draw_aperture(self):
        ...
        for _trace in self.ap_trace:
            ...
            self.ax.plot(_trace[ind_plot], np.arange(self.strimdy)[ind_plot], "w-", lw=1) # lijiao
        self.canvas.draw()
    ...
    def _choose_aperture_image(self):
        ...
        ind_elected = self.tableWidget_files.currentRow()
        print('index of the file', ind_elected)
        if ind_elected>=0:
    ...
    def _add_aperture(self):
        try:
            ...
            _trace = self.trace_local_max(
                ...
                *np.asarray(self.pos_temp[::-1], dtype=int), maxdev=5, fov=5, ntol=5)
            self._trace = _trace
    ...
    def _save_aperture(self):
        ...
        self.ap.polyfitnew(deg = 2, ystart=2000)
        # replace old traces
        ...
        self.ap.polyfitnew(deg=2, ystart=200)
        self._draw_aperture()
        import joblib
    ...
    def _proc_all(self):
        ...
        self.blaze, self.sensitivity = self.ap.make_normflat(self.master_flat-flat_bg, )
        ...
        for i_star, fp in enumerate(fps_star):
            ...
            fp_out = "{}/star-{}-ihdu-{}.dump".format(dirdump, os.path.basename(fp), self.ihdu)
            star = self.read_star(fp)
            hdu = fits.open(fp)
            header = hdu[self.ihdu].header
            hdu.close()
            header0 = self._modify_header(fp)
            isot =  f'{header0["DATE-OBS"]}T{header0["UTC"]}'
            ...
            star1d["UTC-OBS"] = isot
            star1d["jd"] = jd
            star1d["EXPTIME"] = header0["EXPTIME"]
            star1d["header"] = header0
            star1d['STRIM'] = ('[{}:{}, {}:{}]'.format(*self.strimx, *self.strimy), 'PYEXSPEC data reduce')
            star1d["header_ihdu"] = header
    ...
    def _proc_lamp(self, fp, nsigma=2.5, verbose=False, wavecalibrate=True, deg=(4,3)):
        ...
        # extract 1d fear
        lamp1d = self.extract_sum(lamp, n_jobs=1)["spec_sum"]
        # remove baseline
        ...
        if wavecalibrate: 
            ...
            pf1, pf2, indselect = thar.grating_equation(
                x, y, z, deg=deg, nsigma=nsigma, min_select=210, verbose=10)
            ...
            pf1 = None; pf2 = None; rms =np.nan; mpflux=None; nlines=None
            wave_solu = None
        if (0.0 < rms < 1) or (wavecalibrate is False):
            ...
            calibration_dict = OrderedDict(
                ...
                pf1=pf1,
                pf2=pf2,
                deg=(deg, 'The degree of the 2D polynomial'),
                mpflux=mpflux,
                blaze = self.blaze,
                # lamp=lamp,
                lamp1d=lamp1d
