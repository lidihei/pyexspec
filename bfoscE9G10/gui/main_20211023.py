...
class UiBfosc(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.trace_handle = []
        self.strimy = [800, 2011] #strim pixal on y axis (ystart, yend) 
        self.strimx = [15, 2021] #strim pixal on x axis (xstart, xend)
        self.strimdy = self.strimy[1] - self.strimy[0]
        self.strimdx = self.strimx[1] - self.strimx[0]
        self.ap_trace = np.zeros((0, self.strimdx), dtype=int)
        self._fear = None
    ...
    def assumption(self):
        test_dir = "/home/lcq/media/backup/216BFOSC/sdOB_216/20211024_bfosc/"
        self._wd = test_dir
        self.lineEdit_wd.setText(test_dir)
        #self._fear = joblib.load("../template/fear_model.dump")
        self._fear = joblib.load("/home/lcq/Documents/pyrafspec/data/template/template_20211023_order9.dump")
        #self.ap = joblib.load("../aperture_template/ap.dump")
        self.ap = joblib.load(f"/home/lcq/media/backup/216BFOSC/sdOB_216/20211023_bfosc/test/ap.dump")
    ...
    def _proc_bias(self):
        ...
        master_bias = np.median(np.array([fits.getdata(fp) for fp in fps_bias]), axis=0)
        xs, xe = self.strimx
        ys, ye = self.strimy
        self.master_bias = master_bias[ys:ye, xs:xe]
    ...
    def _proc_flat(self):
        ...
        master_flat = np.median(np.array([fits.getdata(fp) for fp in fps_flat]), axis=0)
        xs, xe = self.strimx
        ys, ye = self.strimy
        self.master_flat = master_flat[ys:ye, xs:xe]
    ...
    def _proc_fear(self, fp, nsigma=2.5, verbose=False, deg=(4,7)):
        """ read fear """
